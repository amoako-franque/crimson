	...
	return new Promise(async (resolve, reject) => {
		...
		if (!this.errors.length) {
			const incoming = {
				...
				body: this.data.body,
				created_Date: this.data.createdDate,
      ...
      ===============================================*/
			const [{ insertId }] = await db.execute(
				"INSERT INTO posts (title, body, author, created_date) VALUES (?,?,?,?)",
				[incoming.title, incoming.body, incoming.author, incoming.created_Date]
			)
			resolve(insertId)
		} else {
			reject(this.errors)
...
Post.prototype.actuallyUpdate = function () {
	return new Promise(async (resolve, reject) => {
      ...
      ===============================================*/
			await db.execute("UPDATE posts SET title = ?, body=? WHERE _id = ?", [
				incoming.title,
				incoming.body,
				incoming.requestedPostId,
			])
			resolve("success")
		} else {
			resolve("failure")
...
Post.findSingleById = function (id, visitorId = 0) {
    ...
    ===============================================*/
		let [[post]] = await db.execute(
			"SELECT p.title,p._idp.body, p.author, p.created_date, p.username from posts p join users u on p.author = u._id where p._id = ? ",
			[id]
		)
